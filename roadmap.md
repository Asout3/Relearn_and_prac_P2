...
9. **Mutation testing — prove your tests actually catch bugs** — 100% line coverage doesn't mean your tests are meaningful; a test suite can hit every line and still miss real logic bugs. Run a mutation testing tool against one of your Tier 3 contracts (Vault or AMM) — it automatically injects small bugs (`>` flipped to `<`, `+=` changed to `-=`, a `require` deleted) and reruns your test suite against each mutant. Any mutant that survives (your tests still pass despite the injected bug) reveals a real gap in your test suite. Fix the gap by writing a test that specifically kills that mutant. Resource: [Foundry's built-in mutation testing (via `forge test` mutation support docs)](https://book.getfoundry.sh/) or the [Vertigo-eth mutation testing tool](https://github.com/RareSkills/vertigo-eth) — check current Foundry docs first since built-in support has been actively evolving.
...
15. **Brutalize testing on your Yul code** — Solidity doesn't always zero out unused upper bits in a `uint8`, `bool`, or other sub-32-byte type stored in a word; hand-written Yul that silently assumes "clean" bits can pass every normal test and still be wrong. Use Foundry's `vm.brutalizeMemory` (or manually construct dirty-bit test inputs) against the hot function you rewrote in Yul earlier in this tier, and confirm it still behaves correctly with garbage in the unused bits. Resource: [Foundry Book — Cheatcodes Reference, search "brutalize"](https://book.getfoundry.sh/cheatcodes/).
